import { useCreateNewsComment } from '@/entities/comment';
...
export const NewsCommentForm = ({ newsId }: { newsId: string }) => {
  const user = useUser();
  const { mutateAsync: createComment, isPending } = useCreateNewsComment();
  ...
  const onSubmit: SubmitHandler<CommentInput> = (data) => {
    if (user?.userName) {
      createComment({ ...data, cardId: newsId, userName: user?.userName }).finally(onReset);
    }
  };
    ...
    <Card className="drop-shadow-base custom-outline flex min-h-[80px] w-full flex-row gap-4 rounded-xl px-[15px] pt-[10px] pb-[20px]">
      <form onSubmit={handleSubmit(onSubmit)} className="flex w-full flex-row gap-1">
        <Controller
          ...
          render={({ field }) => (
            <Textarea
              label="Добавить комментарий к новости"
              minRows={2}
              maxRows={8}
              radius="sm"
