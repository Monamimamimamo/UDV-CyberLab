...
namespace Core.Cards.Service
{
    public class CommentService(
     ...
     ICardRepository _projectRepository,
     IMapper _mapper)
     : ICommentService
    {
        public async Task<CommentDto> CreateAsync(CommentCreateDto commentDto, Guid authorId)
        {
            var project = await _projectRepository.GetByIdAsync<CardEntity>(commentDto.CardId);
            if (project == null)
            {
                throw new NotFoundException($"Project with id {commentDto.CardId} not found.");
            }
        ...
        {
            var project = await _projectRepository.GetByIdAsync<CardEntity>(projectId);
            if (project == null)
            {
